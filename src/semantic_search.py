...
class SemanticImageIndex:
    ...
    def _fetch_rows_for_paths(self, paths: Sequence[str]) -> List[sqlite3.Row]:
        ...
        for fp in paths:
            ...
            try:
                ...
                rows = self._conn.execute(
                    f"""
                    SELECT file_path, file_signature, dim, embedding, capture_time, camera_model, lens_model, iso
    ...
    def _apply_filters(
        ...
        - date:<prefix>          (e.g. date:2026:05 or date:2026:05:01)
        ...
        for token in parts:
            ...
            if low.startswith("camera:"):
                ...
                if needle:
                    filtered = [r for r in filtered if needle in str(r["camera_model"] or "").lower()]
                continue
            ...
            if low.startswith("lens:"):
                ...
                needle = t.split(":", 1)[1].strip().lower()
                if needle:
                    filtered = [r for r in filtered if needle in str(r["lens_model"] or "").lower()]
                continue
            ...
            if low.startswith("date:"):
                ...
                if pref:
                    filtered = [r for r in filtered if str(r["capture_time"] or "").startswith(pref)]
                continue
            ...
            if low.startswith("city:"):
                ...
                needle = t.split(":", 1)[1].strip().lower()
                if needle:
                    filtered = [r for r in filtered if needle in str(r["city"] or "").lower()]
                continue
            ...
            if low.startswith("admin:"):
                ...
                needle = t.split(":", 1)[1].strip().lower()
                if needle:
                    filtered = [r for r in filtered if needle in str(r["admin1"] or "").lower()]
                continue
            ...
            if low.startswith("country:"):
                ...
                if needle:
                    filtered = [
                        ...
                        for r in filtered
                        if (
                            needle in str(r["country"] or "").lower()
                            or needle in str(r["country_code"] or "").lower()
                        )
                    ]
                continue
            ...
            if m:
                ...
                if key == "iso":
                    filtered = [r for r in filtered if _ok(int(r["iso"] or 0))]
                elif key == "year":
                    filtered = [r for r in filtered if _ok(self._parse_capture_year(str(r["capture_time"] or "")))]
                elif key == "month":
                    filtered = [r for r in filtered if _ok(self._parse_capture_month(str(r["capture_time"] or "")))]
                elif key == "width":
                    filtered = [r for r in filtered if _ok(int(r["width"] or 0))]
                elif key == "height":
                    filtered = [r for r in filtered if _ok(int(r["height"] or 0))]
                continue
    ...
    def _normalize_loose_metadata_query(self, raw: str) -> str:
        ...
        text = re.sub(
            r"\b(camera|lens|city|admin|country|date)\s*=\s*([^\s]+)\b",
            r"\1:\2",
            text,
            flags=re.I,
        )
    ...
    def _auto_match_metadata_keywords(
        ...
        filtered = list(rows)
        remaining: List[str] = []
        metadata_fields = ("city", "admin1", "country", "country_code", "camera_model", "lens_model")
        ...
        for term in semantic_terms:
            ...
            matched_rows = [
                r
                for r in filtered
                if any(needle in str(r[field] or "").lower() for field in metadata_fields)
            ]
