...
class ImageAdjustPanelWidget(QWidget):
  ...
  - ``sliderReleased``: final preview + ``editing_finished`` (XMP write).
    ...
    def __init__(self, parent=None):
        ...
        hint = QLabel("E — hide · drag point curve or use parametric sliders below")
        hint.setStyleSheet("color: #808080; font-size: 10px;")
        hint.setWordWrap(True)
        layout.addWidget(hint)

        self._tone_curve_row: ToneCurveEditorRow | None = None
        _SECTION_KEYS = {
        ...
        self._nr_btn.setToolTip("Chroma-only NLM denoise (luminance preserved)")
        ...
        self._recovery_btn.setToolTip(
            "Match the P-key recovery preview tone as the adjust starting point "
            "(Exposure / Sat / HSL still apply; PV tone sliders switch to standard tone)"
        )
