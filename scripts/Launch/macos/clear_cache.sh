...
mark_next_launch_cold_start() {
  ...
  echo "  Next ./scripts/Launch/macos/launch_dev.sh will skip session restore (one launch)."
}
...
if [ "$FAILED" -eq 1 ]; then
    ...
    echo "For a true UI cold start: use ./scripts/Launch/macos/launch_dev.sh next"
...
echo "Manual override: RAWVIEWER_DISABLE_SESSION_RESTORE=1 ./scripts/Launch/macos/launch_dev.sh"
echo
