...
ensure_openmp_libraw() {
    ...
    if [ ! -x "$REPO_ROOT/scripts/libraw/build_libraw_openmp.sh" ]; then
...
if $PYTHON_BIN build.py --profile "$PROFILE" ${INCREMENTAL:+--incremental} ${RAWVIEWER_BUILD_SKIP_DEPS:+--skip-deps}; then
    ...
    if [ -d "dist/${APP_BUNDLE}" ]; then
        ...
        RELEASE_SRC="${REPO_ROOT}/scripts/Launch/macos/release"
        rm -rf "${RELEASE_DIR}"
        mkdir -p "${RELEASE_DIR}"
        ditto "dist/${APP_BUNDLE}" "${RELEASE_DIR}/${APP_BUNDLE}"
        cp "${RELEASE_SRC}/install_macos_app.sh" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/remove_macos_quarantine.sh" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/uninstall_macos_app.sh" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/Uninstall RAWviewer.command" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/Install RAWviewer.command" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/clear_macos_cache.sh" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/Clear Cache.command" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/Remove Quarantine.command" "${RELEASE_DIR}/"
        cp "${RELEASE_SRC}/macos_release_readme.txt" "${RELEASE_DIR}/Start Here.txt"
        ...
        chmod +x "${RELEASE_DIR}/Install RAWviewer.command" "${RELEASE_DIR}/Remove Quarantine.command"
        chmod +x "${RELEASE_DIR}/clear_macos_cache.sh" "${RELEASE_DIR}/Clear Cache.command"
        ...
        echo "  pixi run python scripts/libraw/check_libraw_parallelism.py <some.CR3>"
