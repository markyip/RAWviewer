...
| 2026-07-02 | 後續調查與修復移至 [`thumbnail-cache-unification-plan.md`](thumbnail-cache-unification-plan.md)（獨立文件，含完整 PR-1～PR-8 記錄）。摘要：foolproof RAW orientation pipeline（單一 make-independent 權威、cache self-heal、`RAW_EXIF_SENSOR_META_VER` bump 至 10）；`build_gallery()` 每次 rebuild 對數千個尚無 metadata 的檔案各自觸發一次 per-path SQLite `SELECT`（`_metadata_exif_for_path` 的 disk fallback），與背景索引的並發寫入互相競爭，實測 7–13 秒的 `[GALLERY] metadata rebuild took N ms` 卡頓 —— 修復後降至遠低於 1 秒；`_nav_prefetch_stages_for_path()` 在非 zoom 情況下仍對鄰近 2 張圖片 eager 請求 `stages={"full"}`（繞過既有 idle-timer），導致連續按方向鍵時每步都觸發一次感應器解析度解碼 —— 已修復為沿用既有 idle-timer 機制。另完成縮圖層級統一（PR-1～PR-4）與儲存空間優化（PR-8：磁碟快取 LRU eviction + WebP 編碼，實測省 26–29%）。|
