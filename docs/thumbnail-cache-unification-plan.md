...
## 8.5 PR-8 — 儲存空間優化（已完成 2026-07-02）

使用者詢問「目標是減少儲存空間，有什麼建議？」後實作，獨立於 PR-1～PR-7 編號但與 PR-7 共用 eviction 機制。

**1. 磁碟快取 LRU eviction**（`image_cache.py`）：先前沒有任何機制限制 thumbnail/grid/preview 磁碟快取的總大小 —— `cleanup_old_cache()` 只清除超過 30 天的項目，長期使用會無上限成長。新增：
- `PersistentThumbnailCache.get_disk_usage_bytes()` / `evict_lru(bytes_to_free)`（base class，grid/preview 共用）。
- `ImageCache.enforce_disk_cache_budget(max_mb=None)`：總量超過 `RAWVIEWER_DISK_CACHE_MAX_MB`（預設 4096 MB）時，按各層佔比evict 回 90% 目標（hysteresis，避免每次啟動都在邊界反覆觸發）。
- 掛載於 `main.py` 既有啟動清理 hook，但透過 `QThreadPool` 背景執行緒（stat 數千個檔案不該卡住主執行緒）。

**2. Preview/Grid 磁碟重複儲存 —— 調查後確認並非實際問題**：磁碟 preview 與 grid 層的目標解析度剛好相同（皆用 `disk_preview_max_edge()`），一度懷疑兩者會各自儲存幾乎相同的 JPEG。實測發現：`put_preview()` 的 `jpeg_data` 參數在全部呼叫點都從未被傳入 —— `disk_preview_cache` 在實務上從未被寫入（`get_preview()` 讀取時已有 fallback 讀 `disk_grid_cache`）。無需修改。

**3. WebP 取代 JPEG（衍生層）**：grid/thumbnail 磁碟層本來就是**重新編碼**產生（resize 後 `.save()`），改用 WebP 是免費的（相同 resize/encode 成本），實測 grid 省 29.4%、thumbnail 省 26.2%（`RAW_Sample` 15 檔）。新增 `common_image_loader.encode_tile_bytes()` 統一所有寫入點（`RAWVIEWER_TILE_CACHE_FORMAT=jpeg` 可全域還原，讀側用 PIL 自動偵測格式，無需 migration）。**PR-7 的 raw embedded JPEG bytes 快取刻意排除在外**（見 §10.1 決策說明）。
...
**PR-8 決策（2026-07-02）**：此快取儲存**原始未修改的 embedded JPEG bytes**，不應轉為 WebP —— 那需要先解碼再重新編碼，違背「零解碼成本擷取」的設計初衷，且會造成二次有損壓縮。WebP 僅用於 grid/thumbnail/preview 這些**本來就要重新編碼**的衍生層（見 §「PR-8 儲存空間優化」，`common_image_loader.encode_tile_bytes()`）。PR-7 實作時可直接複用 PR-8 的 `ImageCache.enforce_disk_cache_budget()` LRU eviction 機制（新增一個 `embedded_jpeg` tier 即可），不需重新設計高水位/清理邏輯。
