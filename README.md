...
1. 從 [Releases](https://github.com/markyip/RAWviewer/releases/latest) 下載 **`RAWviewer-v2.4-macOS.zip`** (Full) 或 **`RAWviewer-v2.4-macOS-Lite.zip`** (Lite) 並解壓縮。
...
| 下載失敗（SSL / 憑證錯誤） | 請更新至 **v2.4** 或更新版本（已隨附 certifi）。若使用公司 VPN 或代理，請將組織的根憑證新增至** keychain 存取**並設定為「永遠信任」，然後重試 |
...
| 在極大資料夾中藝廊滾動卡頓 | 更新到 **v2.4**。若問題持續，執行 **`clear_cache.sh`** 並重新開啟該資料夾 |
| 直式相片縮圖顯示為橫向或顛倒 | 更新到 **v2.4**。若有在修復前快取的舊縮圖，請執行一次 **`clear_cache.sh`** 清除舊快取 |
...
- **macOS Full 完整版:** `dist/RAWviewer-v2.4-macOS.zip`
- **macOS Lite 輕量版:** `dist/RAWviewer-v2.4-macOS-Lite.zip`
