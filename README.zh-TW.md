...
| 變數 | 預設 | 效果 |
|------|------|------|
| `RAWVIEWER_RAW_EDR` | `1` | macOS：**RAW 工作流程** 時 RAW 走 EDR；設 `0` 關閉 |
...
| `RAWVIEWER_RAW_EDR=1` | **預設。** macOS：選 **RAW（高品質）** 時 RAW 走 EDR；`0` 關閉 |
...
**GPU 加速 RAW 解碼**——早期 GPU 解碼可行，但**正確色票**（與現有 LibRaw / 內嵌 JPEG 管線一致）仍待解決。僅在色準與維護成本可接受時才會釋出。

此與 GPU **視埠**（解碼後像素的 OpenGL 縮放 / 平移，正式版預設開啟；`RAWVIEWER_GPU_VIEW=0` 關閉）不同；上述工作鎖定 **RAW 解碼**本身。
