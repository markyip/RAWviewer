...
- **Adjust 面板** — 非破壞顯影：色調、白平衡預設、裁切、Dodge & Burn、修復筆刷、暈影／去霧、Creative LUT 與可儲存的預設；匯出 JPEG／WebP／16 位元 TIFF（**Windows Plus**另提供僅於匯出時套用的 **AI 降噪**，使用 SCUNet；macOS 的 `.app` 不含此選項——見[選 Standard 還是 Plus？](#選-standard-還是-plus)）
...
| 安裝大小（約） | 約 0.8–1 GB | 約 1.5–2 GB + 約 600 MB 模型 | 約 3–4 GB + 約 600 MB 模型 |
| 建議記憶體 | 8 GB | 16 GB | 16 GB + NVIDIA |

**選 Standard**：安裝精簡，以目視篩片為主。**選 Plus (DirectML)**：要 AI 搜尋但不裝 CUDA torch。**選 Plus (CUDA)**：要 NVIDIA GPU demosaic／SCUNet——體積大半來自 PyTorch cu124（約 2.3–2.4 GB），不是應用程式本身。

Windows 安裝程式提供 **Standard**、**Plus (DirectML)** 與 **Plus (CUDA)**。

**macOS 限制 — AI 降噪：** 打包的 macOS Standard／Plus `.app` **不含 PyTorch**，因此匯出選單不會出現 **JPEG／TIFF + AI denoise (SCUNet)**。一般 JPEG／WebP／16 位元 TIFF 匯出仍可用。SCUNet 匯出目前需要 `torch` + `spandrel`（CUDA 或 Apple MPS）；尚未提供 ONNX／Core ML 路徑。含 torch 的 Windows Plus 才會顯示選項，並可在首次使用時下載約 69 MB 權重。
...
| 重開上次資料夾後變慢或退出 | 8 GB 機器請用 **Standard** 或設 `RAWVIEWER_DISABLE_SESSION_RESTORE=1` |
...
| 匯出選單沒有 **AI denoise**（**Full**） | macOS `.app` 預期行為（未打包 PyTorch）。請用 Windows Plus，或在 Mac 使用一般 JPEG／WebP／TIFF 匯出。見[選 Standard 還是 Plus？](#選-standard-還是-plus) |
...
| 重開被終止（終端機顯示 `Killed: 9`） | 可試 **Standard**、`RAWVIEWER_DISABLE_SESSION_RESTORE=1` 或 `RAWVIEWER_ENABLE_SEMANTIC_SEARCH=0` |
...
| 4 | **物件／主體 ML 遮罩** | **中** | L | 僅 Plus（模型大小）；Lite 維持筆刷／幾何 |
...
- **SCUNet AI 降噪匯出：** 發行版僅 **Windows Plus**。macOS `.app` 不含 PyTorch，匯出選單會隱藏該選項（一般 JPEG／WebP／TIFF 仍可用）。
...
- **AI 降噪模型：** [SCUNet](https://github.com/cszn/SCUNet) `scunet_color_real_psnr`，作者 **Kai Zhang 等**（Apache-2.0）——僅於 **Windows Plus** 匯出時套用的神經網路降噪（[論文](https://doi.org/10.1007/s11633-023-1466-0)；權重來自 [KAIR](https://github.com/cszn/KAIR/releases/tag/v1.0)）；macOS `.app` 不含（未打包 PyTorch）
...
- **[spandrel](https://github.com/chaiNNer-org/spandrel)** —— 載入 SCUNet 權重以供匯出降噪（Windows Plus）
