...
> **v2.6 新功能：** **1–5 星評分**（按鍵 **0–5**、圖庫篩選）；**快速 RAW 解碼**（全尺寸約較先前 rawpy 路徑快 **1.4×**）；Nikon **HE/HE*** NEF；暗房色票；可選 GPU demosaic（`RAWVIEWER_PREFER_GPU_DECODE=1`，預設關閉）。完整說明：[`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
