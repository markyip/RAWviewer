...
| 項目 | 狀態 | 說明 |
|------|------|------|
| **匯出時 AI 降噪與放大**（Restormer ONNX / Real-ESRGAN 2×） | **實作中** | Plus 版匯出時透過區塊化 ONNX 模型進行 AI 降噪與 2× 放大 (`src/onnx_restormer.py`) |
| **變形鏡頭比例復原** (Anamorphic Desqueeze) | **實作中** | 提供固定擠壓比例選單（1.33× / 1.5× / 1.6× / 2.0×），將變形鏡頭畫面復原為正確長寬比 |
| **桶狀／枕狀變形校正** (Barrel / Pincushion Distortion) | **實作中** | 提供手動調整滑桿，修正光學桶狀與枕狀鏡頭變形 |
| **HDR 合成** | **規劃中 / 原型** | 對齊曝光、去除鬼影，並透過 Exposure Fusion 融合亮部與暗部 |
| **全景拼接 / HDR 全景** | **規劃中 / 原型** | 特徵匹配、單應性變形與多頻帶無縫融合 |
| **景深對焦疊加** (Focus Stacking) | **規劃中 / 原型** | 特徵對齊、高頻對焦能量比對與多頻帶金字塔融合延伸景深 |
...
| 3 | **物件／主體 ML 遮罩** | **中** | L | 僅 Plus（模型大小）；Standard 維持筆刷／幾何 |
| 4 | **Windows HDR 顯示路徑** | **中** | L | macOS EDR 為 Fast RAW 效能而移除；Windows 仍為 SDR 色調映射 |
| 5 | **在 Fast RAW 下恢復 macOS EDR** | **低偏中** | L | 先前與快速載入管線衝突；需要不犧牲速度的設計 |
| 6 | **VLM 輔助自動調整** | **低偏中** | L | 產品＋模型／API 範圍（例如本機 Ollama）；非僅編輯器管線問題 |
| 7 | **Google Drive 瀏覽／編輯／XMP 同步** | **中** | L | 本機快取同步（下載 → 既有管線 → 上傳 XMP／匯出）；OAuth＋虛擬資料夾 session |
